...
\begin{itemize}
  \item $\{W\}$ is inertial and right-handed; $+z_W$ points upward.
  \item $\{B\}$ is body-fixed and right-handed; $+x_B$ points forward, $+y_B$
    points to port (left), and $+z_B$ points upward.
\end{itemize}
All angles follow the right-hand rule. The yaw angle $\psi$ is a rotation about
$+z_W$; positive yaw is counter-clockwise when viewed from above.

\paragraph{Marine-convention note.}
Fossen and much of the marine literature use NED/FRD conventions (world $z$
down, body $y$ starboard, body $z$ down). This document and the current ROS~2
implementation use ENU/FLU-style conventions to match standard ROS frames. When
comparing formulas, the following constant transforms are useful:
\[
...
magnitude $T$ and a steering angle $\delta$ relative to $+x_B$ (positive CCW).
...
\[
  \dot{\mathbf{p}} =
  \mathbf{R}(\mathbf{q}_{WB})\mathbf{v}, \qquad
...
\begin{itemize}
  ...
  \item Automated tests exist for the B-spline utilities and basic simulator
    sign/consistency checks, but the controller still lacks direct unit tests.
\end{itemize}
...
Quaternion kinematics for body-rate $\boldsymbol{\omega}$ are
\begin{equation}
...
and $\mathbf{a}=\boldsymbol{\phi}/\theta$. The exponential map gives the delta
quaternion
\begin{equation}
  \Delta\mathbf{q}=
  \begin{bmatrix}
...
Runge--Kutta--Munthe-Kaas (RKMK) methods integrate on the Lie algebra and map
back to the group with the exponential map. A first-order update uses
\begin{equation}
